...
set JAVA_VERSION=17
set APP_VERSION=0.1
set MAIN_JAR=RoflBoard-%APP_VERSION%.jar
...
xcopy /S /Q build\jmods\* target\installer\input\libs\
copy build\libs\%MAIN_JAR% target\installer\input\libs\
...
"%JAVA_HOME%\bin\jdeps" ^
  ...
  --print-module-deps target\installer\input\libs\%MAIN_JAR% > temp.txt
...
call "%JAVA_HOME%\bin\jpackage" ^
  ...
  --name RoflBoard ^
  --main-class ru.rofleksey.roflboard.Main ^
  --main-jar %MAIN_JAR% ^
